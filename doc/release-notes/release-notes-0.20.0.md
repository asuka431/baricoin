...
Baricoin Core version 0.20.0 is now available from:

  <https://download.baricoin.org/>
...
Please report bugs using the issue tracker at GitHub:
  ...
  <https://github.com/asuka431/baricoin/issues>
...
#### Removal of BIP61 reject network messages from Baricoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Baricoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Baricoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Baricoin Core. Baricoin Core logs debug messages
...
- OpenSSL is no longer used by Baricoin Core.  (#17265)

- BIP70 support has been fully removed from Baricoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/asuka431/baricoin/issues/16599),
  [PR #16702](https://github.com/asuka431/baricoin/pull/16702), and the
  `baricoind help` for more information.  This option is experimental and
...
- All custom settings configured when Baricoin Core starts are now
...
- The "Start Baricoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://baricoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Baricoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Baricoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/baricoin-core/HWI) for
  signing.  Future versions of Baricoin Core should support a GUI option
...
- Baricoin Core's automatically-generated source code documentation is
...
- The `baricoin-cli` utility used with the `-getinfo` parameter now
...
- The `baricoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `baricoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_baricoin-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_baricoin` binary (hebasto)
...
- #18702 Fix ASLR for baricoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate baricoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest baricoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_baricoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (baricoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone baricoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Baricoin Core files (hebasto)
...
- #17119 Fix broken baricoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from baricoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.baricoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to baricoin-wallet (hel-o)
...
- #16115 On baricoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from baricoin-cli -getinfo (malevolent)
