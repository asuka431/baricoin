 ...
 */
static const char* BARICOIN_PID_FILENAME = "baricoind.pid";

static fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", BARICOIN_PID_FILENAME)));
}
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", BARICOIN_PID_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/asuka431/baricoin>";
...
/** Sanity checks
 *  Ensure that Baricoin is running in a usable environment with all
...
{
    // Make sure only a single Baricoin process is using the data directory.
...
bool AppInitMain(NodeContext& node)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", BARICOIN_CONF_FILENAME));
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if baricoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if baricoin is started while in a temporary directory.\n",
