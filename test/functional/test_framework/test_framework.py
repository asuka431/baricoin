...
class BaricoinTestFramework(metaclass=BaricoinTestMetaClass):
    ...
    def setup(self):
        ...
        self.config = config
        self.options.baricoind = os.getenv("BARICOIND", default=config["environment"]["BUILDDIR"] + '/src/baricoind' + config["environment"]["EXEEXT"])
        self.options.baricoincli = os.getenv("BARICOINCLI", default=config["environment"]["BUILDDIR"] + '/src/baricoin-cli' + config["environment"]["EXEEXT"])
    ...
    def add_nodes(self, num_nodes, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [self.options.baricoind] * num_nodes
        if binary_cli is None:
            binary_cli = [self.options.baricoincli] * num_nodes
