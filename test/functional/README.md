...
- Use a module-level docstring to describe what the test is testing, and how it
  is testing it.
- When subclassing the BaricoinTestFramwork, place overrides for the
...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to control whether
  ...
  has 25 mature blocks (25x50=1250 BARI) in its wallet.
...
with the baricoind(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a baricoind.  `P2PInterface`
contains the higher level logic for processing P2P payloads and connecting to
the Baricoin Core node application logic. For custom behaviour, subclass the
...
The [`TestShell`](test-shell.md) class exposes the BaricoinTestFramework
...
Taken from the [python-baricoinrpc repository](https://github.com/jgarzik/python-bitcoinrpc).
...
Basic code to support P2P connectivity to a baricoind.

#### [script.py](test_framework/script.py)
Utilities for manipulating transaction scripts (originally from python-baricoinlib)
